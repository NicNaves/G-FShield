...
function Invoke-DockerNodeCommand {
  param(
    [string]$Image,
    [string]$WorkingDirectory,
    [string]$NodeModulesVolume,
  ...
  & docker @arguments
  if ($LASTEXITCODE -ne 0) {
    throw "Falha ao executar comando em container para $WorkingDirectory"
  }
}
...
function Start-NginxStaticContainer {
  ...
  $containerId = & docker @arguments
  if ($LASTEXITCODE -ne 0) {
    throw "Falha ao iniciar container Nginx $Name"
  }
...
if ($useDockerNode) {
  ...
  $frontEnvironment = @{
    ...
    REACT_APP_API_PROXY_TARGET = "http://host.docker.internal:$ApiPort"
...
"@
  ...
  if ($FrontendMode -eq "Preview") {
    Write-Host "Gerando build otimizado do front..."
    Invoke-DockerNodeCommand -Image $DevNodeImage -WorkingDirectory $frontDir -NodeModulesVolume $frontNodeModulesVolume -HomeVolume $frontHomeVolume -CacheVolume $frontCacheVolume -Environment $frontEnvironment -Command "npm run build"

    Write-Host "Iniciando front local com Nginx..."
    Start-NginxStaticContainer -Name $frontContainerName -BuildDirectory $frontBuildDir -TemplatePath $frontNginxTemplate -PublishedPort $FrontendPort -ProxyTarget "http://${apiContainerName}:$ApiPort" -NetworkName $composeNetworkName | Out-Null
  }
  else {
    Write-Host "Iniciando front local em container..."
    Start-DockerNodeContainer -Name $frontContainerName -Image $DevNodeImage -WorkingDirectory $frontDir -Mounts $frontMounts -Environment $frontEnvironment -PublishedPorts @("${FrontendPort}:${FrontendPort}") -NetworkName $composeNetworkName -ShellCommand $frontShellCommand | Out-Null
  }
...
"@
  ...
  if ($FrontendMode -eq "Preview") {
    ...
    Write-Host "Iniciando front local com Nginx..."
    Start-NginxStaticContainer -Name $frontContainerName -BuildDirectory $frontBuildDir -TemplatePath $frontNginxTemplate -PublishedPort $FrontendPort -ProxyTarget "http://host.docker.internal:$ApiPort" | Out-Null

    $state.processes = @($apiProcess)
    $state.dockerContainers = @(
      [PSCustomObject]@{ name = $frontContainerName; role = "front" }
    )
  }
...
Write-Host "Aguardando front responder em http://localhost:$FrontendPort ..."
try {
  Wait-ForHttp -Url "http://localhost:$FrontendPort"
}
catch {
  if ($useDockerNode -or $FrontendMode -eq "Preview") {
    Print-ContainerLogs -Name $frontContainerName
  }
