...
STATE_FILE="${REPO_ROOT}/.server-dev/processes.env"
...
if [[ "$KEEP_DOCKER" != "true" ]]; then
  echo "Parando stack server do GF-Shield..."
  ...
  echo "Parando banco server do webservice/api..."
  invoke_compose "$REPO_ROOT" "${DB_DOWN_ARGS[@]}"
fi

echo "Ambiente server encerrado."
