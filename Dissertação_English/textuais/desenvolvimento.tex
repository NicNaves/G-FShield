...
Consequently, network traffic, protocols, or logs are not consumed in raw form: they must first be transformed into labeled matrices that satisfy this contract. Generality is therefore an architectural and input-format property, not empirical evidence of performance across heterogeneous sources. Historical CSV files reference \texttt{erenoall.arff}, whereas the current script uses the ERENO~1k split; without a manifest, one configuration cannot be attributed to the other. In the available \texttt{erenoall.arff}, \textit{grayhole} occupies index zero and \textit{normal} index one. Although the name \texttt{normalClass=0} remains in legacy binary routines, that path was disabled before the formal-campaign images were built; active evaluation iterates over all nine ARFF classes and computes macro-F1.
...
During formal search, both monoliths and G-FShield use Weka J48 and validation macro-F1. The best validation solution is then evaluated once on test. This adaptation removes historical classifier, objective-function, and metric-scale differences while preserving distinct comparator algorithms: GR--BF in Monolith~1, GR--IWSS in Monolith~2, and the 24 ranking, controller, and operator combinations in G-FShield. The campaign therefore compares complete solutions but does not causally isolate distribution alone.
