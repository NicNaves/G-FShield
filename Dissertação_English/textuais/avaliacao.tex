...
Because all 208 searches stopped at the same limit, median end-to-end time ranged from 2,706 to 2,847 s by configuration. These values do not demonstrate faster convergence. None of the 24 distributed configurations reached internal F1 $\geq 0.95$. Moreover, G-FShield's internal score is the Java code's legacy binary aggregation, whereas the common final evaluation uses multiclass macro-F1. Time to macro-F1 0.95 cannot therefore be computed from internal events. Measuring it requires evaluating intermediate candidates with the common evaluator, without consulting the test set, and recording the first threshold crossing with a monotonic clock.

Under the same aggregate ceiling, distributed runs used a global median of 3.56 cores and median peak of 5.45, compared with approximately 1.07/1.83 for Monolith~1 and 1.00/1.82 for Monolith~2. Distributed memory had a global median of 5,317~MiB and median peak of 6,757~MiB, compared with median peaks of 1,959 and 1,789~MiB for the monoliths. This quantifies observed consumption but does not prove efficiency: the distributed pipeline aggregates multiple Java processes and messaging infrastructure, and the algorithms' internal objectives are not identical.
...
\textbf{Internal validity.} CPU, memory, CPU set, data, seeds, time limit, and final evaluator were controlled. G-FShield nevertheless selects candidates with the legacy binary F1 and \texttt{normalClass=0}; in the ARFF, index 0 is \textit{grayhole}, while \textit{normal} is index 1. The monoliths use the common macro-F1 during search. This objective mismatch prevents attributing differences solely to architecture. The 12 attempts without a complete solution were rejected and retried; this mechanism may favor runs that can initialize, although it preserves the valid seed matrix.
