...
Como as 208 buscas terminaram pelo mesmo limite, o tempo mediano ponta a ponta ficou entre 2.706 e 2.847 s por configuração. Esses valores não demonstram convergência mais rápida. Nenhuma das 24 configurações distribuídas atingiu F1 interno $\geq 0{,}95$. Além disso, o escore interno do G-FShield é a agregação binária legada do código Java, enquanto a avaliação final comum usa F1 macro multiclasse. Assim, não se calcula tempo até F1 macro 0,95 a partir dos eventos internos. Medi-lo exige avaliar candidatos intermediários com o avaliador comum, sem consultar o teste, e registrar a primeira passagem pelo limiar em tempo monotônico.

Sob o mesmo teto agregado, as execuções distribuídas usaram mediana global de 3,56 núcleos e pico mediano de 5,45, contra aproximadamente 1,07/1,83 no Monólito~1 e 1,00/1,82 no Monólito~2. A memória distribuída teve mediana global de 5.317~MiB e pico mediano de 6.757~MiB, contra picos medianos de 1.959 e 1.789~MiB nos monólitos. Isso quantifica consumo observado, mas não prova eficiência: o pipeline distribuído agrega vários processos Java e infraestrutura de mensageria, e os objetivos internos dos algoritmos não são idênticos.
...
\textbf{Validade interna.} CPU, memória, conjunto de CPUs, dados, sementes, limite e avaliador final foram controlados. Entretanto, o G-FShield ainda seleciona candidatos pelo F1 binário legado com \texttt{normalClass=0}; no ARFF, o índice 0 corresponde a \textit{grayhole} e \textit{normal} é o índice 1. Os monólitos usam F1 macro comum durante a busca. Essa diferença de objetivo impede atribuir diferenças somente à arquitetura. As 12 tentativas sem solução completa foram rejeitadas e repetidas; esse mecanismo pode favorecer execuções que conseguem inicializar, embora preserve a matriz de sementes válidas.
...
\textbf{Validade de conclusão.} O pareamento por semente e a correção de Holm evitam tratar candidatos internos como observações independentes e limitam falsos positivos na família testada. Ainda assim, oito sementes oferecem baixo poder, o baseline é determinístico e a seleção da melhor configuração pela mediana de validação introduz seleção entre 24 alternativas. Não se afirma significância estatística.
