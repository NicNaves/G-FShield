...
Consequentemente, tráfego, protocolos ou \textit{logs} não são consumidos em formato bruto: precisam ser previamente transformados em matrizes rotuladas que satisfaçam esse contrato. A generalidade, portanto, é arquitetural e de formato de entrada, não evidência empírica de desempenho em fontes heterogêneas. Os CSVs históricos referenciam \texttt{erenoall.arff}, enquanto o script atual usa a separação ERENO~1k; sem manifesto, não se atribui uma configuração à outra. Além disso, no \texttt{erenoall.arff} disponível, \textit{grayhole} ocupa o índice zero e \textit{normal} o índice um, em conflito com o nome \texttt{normalClass} usado pelo código. Esse mapeamento precisa ser corrigido pelo rótulo antes de uma nova avaliação.
...
Durante a busca formal, os dois monólitos usam o mesmo avaliador Weka J48 e F1 macro na validação. A solução de melhor validação é então avaliada uma única vez no teste. Essa adaptação elimina as diferenças históricas de classificador e escala das métricas, mas preserva os algoritmos comparadores GR--BF e GR--IWSS. Ela não transforma os monólitos em implementações algorítmicas idênticas ao G-FShield, cuja função objetivo interna legada permanece distinta.
