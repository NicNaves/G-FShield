...
Como comparador externo, \citeonline{Scherer2024IWSHAP} propõem o IWSHAP, que ordena características por valores SHAP e aplica seleção incremental do tipo \textit{wrapper} com XGBoost em tráfego CAN. O artigo relata, para seu experimento de suspensão no X-CANIDS, F1 da classe positiva de aproximadamente 0,9186 com 16 das 688 características. Esses valores são resultados publicados sob protocolo, classificador e escala de dados próprios; por isso, nesta dissertação eles contextualizam a qualidade da seleção, mas não são subtraídos diretamente das métricas do G-FShield. A comparação experimental controlada usa o artefato público fixado em uma revisão, partições comuns de treino, validação e teste e o mesmo avaliador final para todos os subconjuntos.
